#!/usr/bin/env python3
"""Exact replay for the corrected UORC-056 C43B local GLV factorization.

Only public constants, frozen toy curves and independently generated held-out
prime-order j=0 curves are used. No external target or unknown scalar is read.
"""
...
# These rows were independently checked for:
#   #E(F_p)=n prime,
#   ord(G)=n,
...
def analyze_curve(
    ...
    curve, order, generator, shift, beta, lam, half, _values = half_sequence(row)
    one = curve.c(1)
    zero = zero_like(one)
    beta_field = curve.c(beta)
    m = (order - 1) // 2
    if order % 2 == 0 or m % 3:
        raise AssertionError("the order must be odd with three GLV cells per root")
    quotient_dimension = m // 3

    if point_count_j0(curve.p) != order:
        raise AssertionError("declared curve does not have the stated order")
    ...
    if (lam * lam + lam + 1) % order:
        raise AssertionError("lambda is not a nontrivial cubic eigenvalue")

    points = [curve.mul(index, generator) for index in range(1, m + 1)]
    if any(point is None for point in points):
        raise AssertionError("unexpected identity in the half-kernel chart")
    ...
    expected_carry_square = poly_mod(
        poly_mul(poly_mul(t_plus_seven, t_plus_seven), t_plus_seven), kappa
    )
    ...
    for index, (x, y) in enumerate(zip(xs, ys), 1):
        ...
        if poly_eval(carry_root, x ** 3) != curve.c(carry) * (y ** 3):
    ...
    if poly_mod(poly_mul(y_beta, y_beta2), kernel) != poly_mod(
        poly_mul(sector_root, curve_rhs), kernel
    ):
    ...
    if poly_mod(poly_mul(carry_lift, sector_root), kernel) != poly_mod(
        poly_mul(oriented_root, curve_rhs), kernel
    ):
        raise AssertionError("Y_G(X)(X^3+7)=C_G(X^3)J_G(X) failed")

    reversal_root = interpolate(xs, [-value for value in y_values], one)
    ...
    if reversal_product != poly_scale(cubic_product_x, curve.c(-1)):
        raise AssertionError("G reversal does not negate C_G")
...
def build_payload() -> dict[str, Any]:
    ...
    if half_kernel != 3 * glv_orbits:
        raise AssertionError("secp half-kernel is not three GLV blocks")

    payload: dict[str, Any] = {
        "profile_id": "UORC-056-LOCAL-GLV-SECTOR-FACTORIZATION-C43B",
        "schema_version": "2.0",
        "scope": {
            "external_targets_used": False,
            "unknown_production_scalars_used": False,
            "character_grammar_claimed": False,
        },
        "curves": rows,
        "exact_algebra": {
            "kernel": "K_H(X)=kappa(X^3)",
            "carry_root": "C_G(T)=Y_G(X)Y_G(beta X)Y_G(beta^2 X), T=X^3",
            "carry_square": "C_G(T)^2=(T+7)^3 mod kappa(T)",
            "sector_root": "J_G(X)=Y_G(beta X)Y_G(beta^2 X)/(X^3+7)",
            "sector_square": "J_G(X)^2=1 mod K_H(X)",
            "reconstruction": "Y_G(X)(X^3+7)=C_G(X^3)J_G(X) mod K_H(X)",
        ...
        "secp256k1_frontier": {
            ...
            "residual_gauge": "Klein-four sign action per split GLV orbit",
        },
        "aggregate": {
            "curves": len(rows),
            "frozen": len(TOYS),
            "heldout": len(HELD_OUT),
            "carry_value_checks": sum(int(row["carry_value_checks"]) for row in rows),
        ...
        "decision": {
            ...
            "ordered_sector_transport_evaluator_found": False,
        ...
        "successor": {
            ...
            "target": "public unsquared evaluator for J_G(x(Q))",
            "gate": (
                "must be sensitive to the residual Klein-four gauge and avoid "
                "enumerating the quotient roots"
            ),
        },
    }
...
def main() -> None:
    ...
    print("UORC056_LOCAL_GLV_SECTOR_FACTORIZATION_C43B_OK")
