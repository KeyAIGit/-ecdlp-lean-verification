...
class LocalGlvGaugeBreakingC43Tests(unittest.TestCase):
    ...
    def test_validated_corpus_and_exact_boundary(self) -> None:
        correction = self.payload["validation_correction"]
        self.assertTrue(correction["all_declared_rows_recounted"])
        self.assertTrue(correction["all_declared_generator_orders_rechecked"])
        self.assertTrue(correction["all_declared_glv_eigenpairs_rechecked"])
        ...
        self.assertEqual(aggregate["carry_value_checks"], 3792)
        self.assertGreater(aggregate["declared_character_atoms"], 0)
        self.assertGreater(aggregate["valid_character_atoms"], 0)
        self.assertEqual(aggregate["decisive_prime"], 2143)
        self.assertTrue(
            aggregate["decisive_all_targets_absent_from_structural_span"]
        )
        self.assertTrue(aggregate["decisive_all_fitted_quartics_absent"])
        ...
        self.assertTrue(
            decision["declared_structural_glv_dft_character_grammar_closed"]
        )
        self.assertFalse(decision["declared_fitted_quartic_family_uniform"])
    ...
    def test_secp256k1_arithmetic(self) -> None:
        ...
        self.assertTrue(frontier["half_kernel_equals_three_glv_cells"])
