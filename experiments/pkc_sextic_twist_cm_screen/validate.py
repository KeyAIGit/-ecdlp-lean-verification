...
EXPECTED_HASSE_PRIME = 41245443549316649091297836755593555342121
...
def validate(root: Path | None = None) -> None:
    ...
    if payload.get("artifact_id") != "pkc-sextic-twist-cm-screen-v0.2":
        fail("unexpected artifact id")
    if payload.get("status") != "arithmetic_screen_only_non_executable":
        fail("unexpected authority status")
    if payload.get("assurance") != "certificate_replayed":
        fail("assurance drift")
    if payload.get("source_independence") != "not_established":
        fail("source-independence drift")
    if payload.get("disposition") != "property_confirmed_mechanism_missing":
        fail("scientific disposition drift")
    if payload.get("experiments_authorized") is not False:
        fail("experiment authorization must remain false")
    if payload.get("route_promotion") is not False:
        fail("route promotion must remain false")
    if payload.get("claim_boundary") != EXPECTED_CLAIM_BOUNDARY:
        fail("claim boundary drift")
    if payload.get("required_follow_up") != EXPECTED_FOLLOW_UP:
        fail("required follow-up drift")
    if payload.get("issue") != 252:
        fail("issue binding drift")
    if payload.get("auxiliary_curve", {}).get("model") != "y^2 = x^3 + b":
        fail("auxiliary curve model drift")
    ...
    if (trace + 3 * v) % 2 or (trace - 3 * v) % 2:
        fail("CM conjugate traces are nonintegral")
    ...
    if int(cm_record["base_trace"]) != trace or int(cm_record["v"]) != v:
        fail("base CM reconstruction drift")
    ...
    if (
        int(cm_record["selected_order"]) != EXPECTED_ORDER
        or int(cm_record["selected_order_index"]) != 3
        or int(cm_record["selected_trace"]) != traces[3]
    ):
        fail("selected order drift")

    artifact_points = payload["deterministic_points"]
    if len(artifact_points) != 5:
        fail("deterministic point count drift")
    derived_hasse_witnesses: list[dict[str, Any]] = []
    for expected_index, row in enumerate(artifact_points):
        ...
        derived_hasse_witnesses.append({
            "label": expected_label,
            "full_multiple_is_identity": is_infinity(jacobian_multiply(EXPECTED_ORDER, point, p), p),
            "cofactor_multiple_is_identity": is_infinity(
                jacobian_multiply(EXPECTED_ORDER // EXPECTED_HASSE_PRIME, point, p),
                p,
            ),
        })
    ...
    if root_factors.get(EXPECTED_HASSE_PRIME) != 1:
        fail("Hasse witness prime must divide M exactly once")
    if EXPECTED_ORDER % EXPECTED_HASSE_PRIME:
        fail("Hasse witness prime does not divide M")
    if not prime_cache.get(EXPECTED_HASSE_PRIME):
        fail("Hasse witness factor is not certified prime")
    q_squared = EXPECTED_HASSE_PRIME * EXPECTED_HASSE_PRIME
    sixteen_p = 16 * p
    candidate_trace = traces[3]
    candidate_trace_squared = candidate_trace * candidate_trace
    four_p = 4 * p
    if q_squared <= sixteen_p:
        fail("Hasse uniqueness inequality q > 4*sqrt(p) failed")
    if candidate_trace_squared > four_p:
        fail("candidate order is outside the Hasse interval")
    if any(
        not row["full_multiple_is_identity"] or row["cofactor_multiple_is_identity"]
        for row in derived_hasse_witnesses
    ):
        fail("Hasse point-order witness failed")

    hasse = payload["hasse_uniqueness_certificate"]
    if hasse.get("theorem") != EXPECTED_HASSE_THEOREM:
        fail("Hasse theorem statement drift")
    expected_hasse_scalars = {
        "candidate_order": EXPECTED_ORDER,
        "candidate_trace": candidate_trace,
        "candidate_trace_squared": candidate_trace_squared,
        "four_p": four_p,
        "large_prime_factor": EXPECTED_HASSE_PRIME,
        "large_prime_factor_exponent": 1,
        "cofactor": EXPECTED_ORDER // EXPECTED_HASSE_PRIME,
        "q_squared": q_squared,
        "sixteen_p": sixteen_p,
    }
    for field, expected in expected_hasse_scalars.items():
        if int(hasse[field]) != expected:
            fail(f"Hasse certificate field drift: {field}")
    expected_hasse_flags = {
        "candidate_in_hasse_interval": True,
        "q_divides_candidate_order": True,
        "q_gt_four_sqrt_p": True,
    }
    for field, expected in expected_hasse_flags.items():
        if hasse.get(field) is not expected:
            fail(f"Hasse certificate flag drift: {field}")
    if hasse.get("point_witnesses") != derived_hasse_witnesses:
        fail("Hasse point witnesses drift")
    if hasse.get("conclusion") != "selected_order_equals_curve_cardinality":
        fail("Hasse uniqueness conclusion drift")

    smooth = payload["smooth_part"]
    smooth_factors = parse_int_map(smooth["factorization"])
    expected_smooth_factors = {
        q: exponent
        for q, exponent in EXPECTED_ROOT_FACTORS.items()
        if q != EXPECTED_HASSE_PRIME
    }
    if smooth_factors != expected_smooth_factors:
        fail("smooth-part factorization drift")
    expected_smooth_value = multiply_factorization(expected_smooth_factors)
    if expected_smooth_value != int(smooth["value"]):
        fail("smooth-part factorization mismatch")
    if smooth.get("log2") != format(math.log2(expected_smooth_value), ".15f"):
        fail("smooth-part log2 mismatch")

    frow = payload["frobenius_eisenstein"]
    frobenius = int(frow["a"]), int(frow["b"])
    if int(frow["norm"]) != p or int(frow["trace"]) != traces[3]:
        fail("recorded Frobenius semantics drift")
    ...
    getcontext().prec = 50
    relation_rows = payload["relation_yield_screen"]
    if len(relation_rows) != len(EXPECTED_RELATION_FACTORS):
        fail("relation-yield row count drift")
    seen_arities: set[int] = set()
    for row in relation_rows:
        m = int(row["m"])
        if m in seen_arities or m not in EXPECTED_RELATION_FACTORS:
            fail("relation-yield arity drift")
        seen_arities.add(m)
        factors = parse_int_map(row["factorization"])
        if factors != EXPECTED_RELATION_FACTORS[m]:
            fail("relation-yield factorization drift")
        ...
        if row.get("log2_divisor") != format(math.log2(divisor), ".15f"):
            fail("relation-yield log2 divisor mismatch")
    if seen_arities != set(EXPECTED_RELATION_FACTORS):
        fail("relation-yield arity coverage drift")
