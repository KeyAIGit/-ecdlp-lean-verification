...
| Research Engine v0 | Boolean admissibility before scoring, mechanism/validator intake, preregistered expected-information-gain ordering, raw-artifact validator replay, deterministic outcome classification, immutable events, explicit decision deltas, and separate threat-model/decision/evidence axes | `repo/RESEARCH_ENGINE_V0.json`, `experiments/engine/outcomes/`, `experiments/engine/runs/`, `data/research_engine_state.json` | Edit policy and append events; regenerate state. A pending mechanism or validator cannot be selected, and generated evidence cannot promote a route. |
...
| Reproducible experiments | Non-kernel scripts, manifests, immutable outcomes, and measured evidence | `experiments/` | Measurements are evidence, never proofs. Only the selected bounded sequence may run; promotion remains separately gated. |
