...
```text
experiments/parity_lift_000/uorc056_primal_orientation_branch.py
```

uses six frozen toy curves with prime subgroup orders
...
```text
all 1212 nonzero points,
all anti-Kummer pairs,
the finite-etale square equation,
the unsigned even/odd product collapse,
G -> -G,
the local translation recurrence,
all generator replacements for orders 19, 31, 67,
selected generator replacements for orders 271, 397, 433,
alternate square-root branches.
```

The replay is evidence for the exact finite-field identities. The general
theorems above are proved algebraically in this memo and do not depend on the
toy screen.
