...
import pickle
pickle.dump((cols,mats,{str(k):int(v) for k,v in tvec.items()}), open('r8_cache.pkl','wb'))
print(f"cached ({time.time()-t0:.0f}s)",flush=True)

from fractions import Fraction as Fr
# build sparse rows over all monomials
allm=set(tvec); [allm.update(pv) for pv in mats]
rows=[]  # (dict col->Fr, rhs Fr)
for m_ in allm:
    row={}
    for k_,pv in enumerate(mats):
        if m_ in pv: row[k_]=Fr(int(pv[m_]))
    rows.append((row, Fr(int(tvec.get(m_,0)))))
print(f"{len(rows)} rows x {len(cols)} cols; eliminating ({time.time()-t0:.0f}s)",flush=True)
# Gaussian elimination (column pivoting), sparse
pivots={}   # col -> row index in 'piv'
piv=[]      # reduced pivot rows
ncols=len(cols)
def add_row(r, rhs):
    r=dict(r)
    while r:
        col=min(r)  # smallest col index as pivot candidate
        if col in pivots:
            pr, prhs = piv[pivots[col]]
            f=r[col]/pr[col]
            for k2,v2 in pr.items():
                r[k2]=r.get(k2,Fr(0))-f*v2
                if r[k2]==0: del r[k2]
            rhs=rhs-f*prhs
        else:
            pivots[col]=len(piv); piv.append((r,rhs)); return None
    # r empty
    if rhs!=0: return 'INCONSISTENT'
    return None
inc=False
for r,rhs in rows:
    if add_row(r,rhs)=='INCONSISTENT': inc=True; break
if inc:
    print("NO SOLUTION (inconsistent) -> tail NOT in span",flush=True)
else:
    # back-substitute: particular solution with free vars=0
    x=[Fr(0)]*ncols
    for col in sorted(pivots, reverse=True):
        r,rhs=piv[pivots[col]]
        s=rhs-sum(v2*x[k2] for k2,v2 in r.items() if k2!=col)
        x[col]=s/r[col]
    vals=[sp.Rational(v.numerator,v.denominator) for v in x]
    combo=sum(vals[k_]*cols[k_][1]*cols[k_][2] for k_ in range(ncols) if vals[k_]!=0)
    ok=NF(E-combo)==0
    print(f"SOLUTION found; NF(E-combo)==0: {ok} ({time.time()-t0:.0f}s)",flush=True)
    import collections
    agg=collections.defaultdict(lambda: sp.Integer(0))
    for k_ in range(ncols):
