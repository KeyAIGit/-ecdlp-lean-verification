/-
NON-BUILT DRAFT — RH multiplicity / divisor package (S1-MULTIPLICITY, plus the
divisor half of the S1-CONJ exit), statements M1–M17.

This file implements the contract
`domains/riemann-hypothesis/MULTIPLICITY_CONTRACT.md` (DRAFT v2, 2026-08-07,
statement surface M1–M17 after adversarial findings A1–A10). ALL THIRTY-FOUR
`theorem` signatures below are reproduced CHARACTER-IDENTICALLY (modulo line
wrapping) from the `lean` statement blocks of that contract's §2 — including the
four M15 specializations `riemannXi_divisor_univ_conj`,
`riemannXi_divisor_strip_conj`, `riemannXi_divisor_univ_one_sub_conj` and
`riemannXi_divisor_strip_one_sub_conj`, which the contract previously mandated in
prose only and now spells explicitly with M14's carrier sets (`Set.univ` and
`{z : ℂ | 0 < z.re ∧ z.re < 1}`) and M14's `_univ_`/`_strip_` naming. Verified by
mechanical comparison this session: 34 signatures on each side, zero mismatches.
No line-range locator into the contract is cited here: the
contract is itself under revision for independent acceptance, so its line
numbers are not a stable address. Beyond those four signatures, only the proof
...
declares `defaultTargets = ["Ecdlp", "ResearchOS"]` and the two `lean_lib`
roots of the same names; this file is under neither), outside the no-`sorry`
gate's scan surface, and outside the result registries. **CI DOES NOT
ELABORATE THIS FILE AS IT STANDS.** No CI job builds it and none typechecks
it: `.github/workflows/ci.yml` runs `lake build` (:420), the two
`lake env lean` axiom audits (:428, :438) and a non-blocking stem typecheck
loop restricted to `Ecdlp/Targets/*.lean` (:451), and no job under
`.github/workflows/` names this path. **The Lean kernel has NOT
checked one line of it**, and no future CI run will check it until a promoted
copy is placed under a lake target by a separate promotion change. The static
reviews recorded in this file and in `drafts/README.md` are human/model
reading, NOT CI evidence. No `sorry`, no `admit`, no `axiom`, no
`native_decide` appears; but "no `sorry`" is not "proved" — under the one
invariant only a green `lake build` on the promoted built copy is a verdict.
Nothing here is claimed proved, and nothing here bears on the truth of the
Riemann Hypothesis (see the contract's §Claim boundary: M7/M8 say the four
points `ρ, 1-ρ, conj ρ, 1-conj ρ` carry equal multiplicity, which is
compatible with every conceivable zero configuration, including all
counterexamples to RH).

AUTHORITY FOR THIS LANE. The queue that governs RH work is
`tasks/RIEMANN_HYPOTHESIS.md`. Under its current dated decision `RH-002`
remains the sole ACTIVE task, no theorem-bearing route is selected, and no
route execution is authorized. `repo/ECDLP_DECISION_SUBSTRATE.json` governs the
ECDLP lane and is NOT the authority here; nothing in this file may be read as
deriving authorization from it. This file is therefore an OFFERED ARTIFACT —
a statement implementation prepared for later independent review — and NOT an
active task, NOT a selected route, and NOT a claim that `S1-MULTIPLICITY` or
`S1-CONJ` is being worked. Its two gates are independent: (i) independent
acceptance of the contract's statement surface, which is a review step with no
kernel content, and (ii) SEPARATELY AND LATER, kernel promotion of a built
copy into a lake target. Neither has happened, and (i) never implies (ii).

PACKAGE DEPENDENCIES (repo modules, referenced by name, never re-proved).

  (1) BUILT AND MERGED — `ResearchOS/AnalyticNumberTheory/RiemannHypothesis/Xi.lean`
      (RH-007, PR #304). Consumed: `riemannXi` (:41), `differentiable_riemannXi`
      (:46), `riemannXi_one_sub` (:61), `riemannXi_zero` (:72),
      `analyticOrderAt_riemannXi_eq_riemannZeta` = X11 (:248, hypotheses
      `0 < s.re`, `s.re < 1` verbatim).

  (2) BUILT AND MERGED — `…/TargetBridge.lean` (RH-004, PR #299, P1–P5).
      Consumed only transitively (Xi and Conj both import it); no bridge
      theorem is invoked by name below.

  (3) BUILT AND MERGED — `…/Conj.lean` (the conjugation package, RH-008,
      PR #307, commit `c277b86`, on `main` and imported from `ResearchOS.lean`).
      Consumed: `analyticOrderAt_riemannZeta_conj` (:440) and
      `analyticOrderAt_riemannXi_conj` (:452). These are PACKAGE
      prerequisites, NOT pinned Mathlib, and are declared as such.

STATEMENTS THAT CONSUME THE MERGED CONJUGATION PACKAGE (taken from the
contract's §3 dependency map). An earlier revision of this header listed these
as BLOCKED ON PR #306. That blocker is GONE and the citation was wrong twice
over: PR #306 is closed unmerged and must never be cited as provenance, and the
conjugation package these statements need is merged as PR #307 and is on
`main`. Nothing below waits on an unmerged PR:

    M5   `analyticOrderAt_riemannXi_one_sub_conj`        ← Conj.lean:452
    M6   `analyticOrderAt_riemannZeta_one_sub_conj`      ← Conj.lean:440
    M7   `analyticOrderAt_riemannXi_fourfold`            ← M5 + Conj.lean:452
    M8   `analyticOrderAt_riemannZeta_fourfold`          ← M6 + Conj.lean:440
    M15  `riemannXi_divisor_conj`,
         `riemannXi_divisor_univ_conj`,
         `riemannXi_divisor_strip_conj`,
         `riemannXi_divisor_one_sub_conj`,
         `riemannXi_divisor_univ_one_sub_conj`,
         `riemannXi_divisor_strip_one_sub_conj`          ← M5 + Conj.lean:452
    M17b `riemannZeta_divisor_strip_conj`                ← Conj.lean:440
    M17c `riemannZeta_divisor_strip_one_sub_conj`        ← M6 + Conj.lean:440

  M1–M4, M9–M14, M16, M16', M16'' and M17a consume no conjugation-package
  lemma at all; they need only `main` + the pin. The contract's landing order
  (`M1 M2 M3 M4` → `M9 M10 M11 M12 M13 M14` → `M16 M16' M16'' M17a` →
  `M5 M6 M7 M8 M15 M17b M17c`) no longer contains a wait state; with the
  conjugation package merged it is a review-convenience ordering only. What
  still gates this file is NOT a package prerequisite: it is independent
  contract acceptance, and separately and later, kernel promotion.

SCOPE / BOUNDARY (contract §Claim boundary, reproduced so the file carries it).
Zero `def`s. No zero enumeration, no counting function, no growth or Hadamard
bound, no Li coefficients, no simplicity claim, no `RiemannHypothesis`
mention. Every ζ statement (M4, M6, M8, M16–M17) carries `0 < s.re` and
`s.re < 1`: X11 holds only where the Γℝ cofactor is a nonvanishing unit, and
the global form of M4 is FALSE — `riemannZeta (-2) = 0` while
`riemannZeta 3 ≠ 0` and `1 - (-2) = 3` (contract M4 note; death condition 6).
No sharpening of that note to "the trivial zeros are simple" (finding A7).
`S1-GLOBAL-ZEROS` is a different barrier and is untouched.

Pinned Mathlib: fabf563a7c95a166b8d7b6efca11c8b4dc9d911f (v4.31.0), toolchain
`leanprover/lean4:v4.31.0`, re-verified this session by
`git -C /workspace/leanprover-community/mathlib4 rev-parse HEAD`. Every lemma
name, namespace, implicit/explicit binder shape and hypothesis form cited
below was re-read at that exact revision. THREE NAMESPACE TRAPS were
confirmed or newly found while doing so, and all three are load-bearing:

  * (contract §1, confirmed) `namespace MeromorphicOn` spans
    Divisor.lean:28-468 and lines :71 / :177 carry no `_root_` prefix
    (contrast :83, which writes `_root_.divisor_sphere_support_finite`), so
    the true names are `MeromorphicOn.AnalyticOnNhd.divisor_apply` and
    `MeromorphicOn.AnalyticOnNhd.divisor_nonneg`, and dot notation
    `hf.divisor_apply` on an `AnalyticOnNhd` hypothesis does NOT resolve.
    Every use below is fully qualified.

  * (contract §0 / §M12 / pinned-API table, confirmed) `analyticOrderAt_eq_top_iff_eq_zero`
    (Order.lean:687) sits INSIDE `namespace AnalyticOnNhd`, which opens at
    Order.lean:575 and closes at :700 — the same namespace the contract
    already invokes for :614/:624; :693 writes `_root_.IsOpen.…` explicitly,
    which is the proof that the namespace is open there. Its true name is
    therefore `AnalyticOnNhd.analyticOrderAt_eq_top_iff_eq_zero`, which is how
    the contract's M12 skeleton and pinned-API table now spell it. Written
    qualified at M12. Note that despite living in that namespace the lemma takes
    NO `AnalyticOnNhd` argument — its hypotheses are `[PreconnectedSpace 𝕜]` and
    pointwise analyticity everywhere, `∀ z₀, AnalyticAt 𝕜 f z₀` — so dot notation
    on an `AnalyticOnNhd` hypothesis does NOT resolve either.

  * (contract §0 / §M9, confirmed) `analyticOnNhd_univ_iff_differentiable`
    (CauchyIntegral.lean:678) sits inside `namespace Complex`
    (CauchyIntegral.lean:173-770; :625 writes
    `protected theorem _root_.DifferentiableOn.analyticAt`, which is the proof
    that the namespace is open there). Its true name is
    `Complex.analyticOnNhd_univ_iff_differentiable`; both in-tree call sites
    (Meromorphic/Complex.lean:21, LSeries/ZMod.lean:525) reach it only through
    an `open Complex`. Written qualified at M9.

CONVENTION OF THIS LANE: where a step is low-confidence it is KEPT and
annotated with a concrete named fallback alternate. The bridge, xi and
conjugation drafts all did this and CI later consumed several of the recorded
fallbacks. `LOWCONF` marks the steps most likely to need one.
-/
...
theorem riemannXi_divisor_strip_conj (s : ℂ) :
  ...
  -- `0 < z.re ∧ z.re < 1`, so `simp only` closes the `Iff` by `rfl`. Fallback if
  -- it does not:
  --   exact Iff.rfl   (append after the `simp only`)
...
theorem riemannXi_divisor_strip_one_sub_conj (s : ℂ) :
  ...
  · simp only [Set.mem_setOf_eq, Complex.conj_re]
    -- OBLIG S1M-15b (LOW): after `Complex.conj_re` both sides are literally
    -- `0 < z.re ∧ z.re < 1`, so `simp only` closes the `Iff` by `rfl` only if it
    -- can reduce `P ↔ P` to `True` (`iff_self` is NOT in a `simp only` set).
    -- Fallback: append
    --   exact Iff.rfl
