...
builds only the `Ecdlp` and `ResearchOS` roots), outside the no-`sorry` gate's
scan surface, and outside the result registries. **The Lean kernel has NOT
checked one line of it.** No `sorry`, no `admit`, no `axiom`, no
...
PACKAGE DEPENDENCIES (repo modules, referenced by name, never re-proved).
  ...
  (3) CI-GREEN BUT NOT YET ON `main` — `…/Conj.lean` (the conjugation package,
      PR #306). Consumed: `analyticOrderAt_riemannZeta_conj` (:440) and
      `analyticOrderAt_riemannXi_conj` (:452). These are PACKAGE
      prerequisites, NOT pinned Mathlib, and are declared as such.

STATEMENTS BLOCKED ON PR #306 (taken from the contract's §3 dependency map and
its status block at :12-14 and :61-66). These are written here in full, since
`Conj.lean` is CI-green and will be merged before any promotion of this file,
but they are UNBUILDABLE until it lands on `main`:
  ...
  M1–M4, M9–M14, M16, M16', M16'' and M17a are independent of PR #306 and can
  be landed against `main` + the pin. Contract landing order: `M1 M2 M3 M4` →
  `M9 M10 M11 M12 M13 M14` → `M16 M16' M16'' M17a` → *wait for #306* →
  `M5 M6 M7 M8 M15 M17b M17c`.
...
confirmed or newly found while doing so, and all three are load-bearing:
  ...
  * (NEW, not recorded in the contract) `analyticOrderAt_eq_top_iff_eq_zero`
    ...
    therefore `AnalyticOnNhd.analyticOrderAt_eq_top_iff_eq_zero`, and the
    contract's M12 skeleton spells it unqualified. Written qualified at M12.

  * (NEW, not recorded in the contract) `analyticOnNhd_univ_iff_differentiable`
...
import ResearchOS.AnalyticNumberTheory.RiemannHypothesis.Conj   -- PR #306: the Z9 order legs
...
/-! ## Block C — the composite ρ ↦ 1 − conj ρ (barrier exit item (b)) `[#306]`

## M5. ξ composite order transport `[#306]`

BLOCKED ON PR #306: consumes `analyticOrderAt_riemannXi_conj`
(repo:`Conj.lean:452`), a PACKAGE prerequisite, NOT pinned Mathlib. -/
...
/-! ## M6. ζ composite order transport, open strip only `[#306]`

BLOCKED ON PR #306: consumes `analyticOrderAt_riemannZeta_conj`
(repo:`Conj.lean:440`).
...
/-! ## M8. ζ fourfold order action, open strip only `[#306]`
...
/-! ## M12. ξ has finite order everywhere `[PIN]` — **THE MAIN OBLIGATION (S1M-FIN)**
...
theorem analyticOrderAt_riemannXi_ne_top (z : ℂ) : analyticOrderAt riemannXi z ≠ ⊤ := by
  ...
  have hzero : riemannXi = 0 :=
    ...
    -- NAMESPACE (header trap #2, NOT recorded in the contract): Order.lean:687 is
    -- inside `namespace AnalyticOnNhd` (:575-:700), so the contract's unqualified
    -- spelling would not resolve. Signature re-read verbatim at the pin:
  ...
  -- (repo:`Conj.lean` Z3, OBLIG S1C-7) and is CI-green, which is the strongest
  -- evidence available without a toolchain.
  -- FALLBACK ROUTE (contract-corrected under finding A3 — it needs a FINITE-ORDER
  -- WITNESS first; the fifth argument of Order.lean:624 is NOT free):
...
POINTWISE FINITE ORDER as its only hypothesis. -/
...
/-! ## M15. Divisor conjugation and composite invariance `[#306]`

BLOCKED ON PR #306 (all six declarations): the ξ-conjugation order leg is
repo:`Conj.lean:452` and the composite leg is M5. Proof pattern is M14's with
`analyticOrderAt_riemannXi_conj` resp. M5 in place of M3; the obligation class
is S1M-14a / S1M-14b, plus PR #306 landing.

The `Set.univ` and open-strip instances are the ones the contract's M15 text
requires ("with the same `Set.univ` and open-strip instances as M14"); the
strip is conj-stable because `((starRingEnd ℂ) z).re = z.re`
(`Complex.conj_re`, Data/Complex/Basic.lean:467).
...
theorem riemannXi_divisor_strip_conj (s : ℂ) :
  ...
  -- `simp only` closes the `Iff` by `rfl`. Fallback if it does not:
...
/-! ## M16'. ζ has finite order on Ω `[PIN]` — free from X11 + M12

This costs NOTHING NEW: X11 (repo:`Xi.lean:248`) transports M12's finiteness
from ξ to ζ inside the strip. No nonvanishing witness for ζ on Ω has to be
produced, and in particular NO zero-free region, NO growth bound and NO
unproved analytic input enters. M12 and M16' are the package's only finiteness
claims. -/
...
M17b and M17c are BLOCKED ON PR #306 (`analyticOrderAt_riemannZeta_conj`,
repo:`Conj.lean:440`, plus M6). M17a is not. -/
...
order, is effective, has finite order everywhere, and its support is exactly
the ξ zero set) AND M16–M16'' for ζ on the open strip (the same four
...
WHAT IT DOES NOT. It does not on its own close either barrier row.
`S1-CONJ` closes only if BOTH this package AND PR #306 land, because M15/M17c
themselves consume repo:`Conj.lean:440/452`. `S1-MULTIPLICITY`'s row is scoped
to THIS REPOSITORY's ζ/ξ layer: generic pinned Mathlib machinery lowers the
cost of an exit but NEVER retires a row, and the earlier claim that the row is
"stale" is retracted (finding A4, death condition 9). An open row is not by
itself authorization to work a route (death condition 8;
`repo/ECDLP_DECISION_SUBSTRATE.json` discipline) — this file is a statement
implementation, not a claim that `S1-MULTIPLICITY` is the selected target.

NO ZERO ENUMERATION (no index type, no ordering, no `ρ : ℕ → ℂ`, no `Finset` of
zeros, no `⋃`/`∑` over zeros; the divisor is a `locallyFinsuppWithin` object
with LOCALLY finite support, and on `Set.univ` and on Ω that support is
infinite and never a `Finsupp`). NO COUNTING (no `N(T)`, no `|ρ| ≤ T`
