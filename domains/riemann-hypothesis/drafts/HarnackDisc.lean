/-
DRAFTS-LANE DRAFT — Harnack double inequality on a disc (upstream-pool item), H1-H5.

This file is a NON-BUILT Lean source draft of
`domains/riemann-hypothesis/HARNACK_CONTRACT.md` (DRAFT v1, 2026-08-07, offered
for stage-one statement acceptance only). It is NOT part of any lake target
(`lakefile.toml` declares `defaultTargets = ["Ecdlp", "ResearchOS"]`; nothing
in this directory is under either), it is outside the no-sorry gate's scan
surface, and **the Lean kernel has NOT checked it** — no CI workflow elaborates
this path, and no green CI run on any PR carrying this file is evidence about
its contents. Every proof body below is complete as written (no `sorry`, no
`admit`, no `axiom`), but completeness here is a static-reading claim only;
the kernel's verdict would come from a separate promotion or upstream-Mathlib
change that this file does not effect.

Statement surface: exactly the five public signatures H1-H5 of the contract,
character-identical to its §1/§2 `lean` blocks. Zero `def`s. Per the contract's
claim boundary: this package consumes pinned Mathlib only, closes no
capability-map row, selects no route, and carries no RH content — ζ, ξ, and
their zeros appear nowhere below.
...
HEAD`; every `file:line` locator below was re-read at that pin this session.
-/
