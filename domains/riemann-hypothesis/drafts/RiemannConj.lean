/-
NON-BUILT DRAFT — RH conjugation-symmetry package (S1-CONJ), statements Z1–Z9.

This file implements the frozen contract
`domains/riemann-hypothesis/CONJ_SYMMETRY_CONTRACT.md` (draft v2, 2026-08-06,
adversarial verdict SOUND_WITH_FIXES, fixes F1–F4 folded in; F1's
`linear_combination` closer is re-derived below — see the Z3 inline note).

It is NOT part of any lake target: it must not be imported from `Ecdlp.lean`
or any built module. The Lean kernel has NOT checked this file; nothing in it
is claimed proved until a built PR is kernel-verified after independent
review, bridge/xi package landing (for Z8 / Z7+Z9-xi), and closure of the
carried preconditions.

PACKAGE DEPENDENCY (a) — built bridge module, ALREADY MERGED on main:
`ResearchOS.AnalyticNumberTheory.RiemannHypothesis.TargetBridge`
(file `ResearchOS/AnalyticNumberTheory/RiemannHypothesis/TargetBridge.lean`)
supplies the two Z8 prerequisites, quoted verbatim from the built module:

  theorem riemannZeta_zero_mem_critical_strip {s : ℂ} (hz : riemannZeta s = 0)
      (htriv : ¬∃ n : ℕ, s = -2 * (n + 1)) : 0 < s.re ∧ s.re < 1                -- P2

  theorem riemannZeta_one_sub_eq_zero_iff {s : ℂ} (h0 : 0 < s.re) (h1 : s.re < 1) :
      riemannZeta (1 - s) = 0 ↔ riemannZeta s = 0                               -- P3

PACKAGE DEPENDENCY (b) — sibling xi draft (future built module Xi):
`domains/riemann-hypothesis/drafts/RiemannXi.lean` supplies `riemannXi` (X1),
used by Z7 and Z9-xi only, quoted verbatim from that draft:

  noncomputable def riemannXi (s : ℂ) : ℂ :=
    (1 + s * (s - 1) * completedRiemannZeta₀ s) / 2

As in the xi draft, this file is written as if those modules' contents are
already available: it references `riemannZeta_zero_mem_critical_strip`,
`riemannZeta_one_sub_eq_zero_iff`, and `riemannXi` by name as if in scope —
they are NOT re-proved or re-defined here. The eventual built PR imports the
built bridge module and the built xi module.

Anti-pitfall compliance (carried from the contract): no proof below uses any
`s ↦ 1 − s` functional-equation fact to obtain conjugation (`1 − s` enters
only in Z8 through bridge P3, after conjugation is independently proved); the
pointwise transfer `Λ = Gammaℝ · ζ` is used only on the open half-plane
`1 < re` under proved nonvanishing (Z3 `key`); every `cpow` conjugation
carries an explicit branch-cut discharge on a real-nonnegative base; no new
definition except the two generic Z9 lemmas (obligation S1C-ORD); no
competing RH proposition.
...
revision this session. Obligations discharged inline: S1C-1 (termwise
Dirichlet conj; the `n = 0` term covered uniformly by `natCast_arg`), S1C-2
(`tsum_star` star/conj mediation; plain `∑'` = `tsum · (unconditional _)`,
InfiniteSum/Defs.lean:160), S1C-3 (realness of `ζ(1)`, mirroring the pin's
own cast walk at ZetaAsymp.lean:431-435), S1C-4 (`{1 < re} ∈ 𝓝 2` glue,
mirroring DirichletContinuation.lean:143), S1C-5 (Z3 `key` rearrangement;
`linear_combination` sign RE-DERIVED, see the Z3 inline note), S1C-6 (Z1
branch-cut/cast dance), S1C-7 (`PreconnectedSpace ℂ` instance for
`eq_of_eventuallyEq`, witnessed in-tree at ZMod.lean:526), S1C-8 (Z4 closing
simp set), S1C-9 (Z6 `LeftInverse`/`RightInverse` bundling), S1C-10 (Z7
normal-form alignment), S1C-ORD (the two generic Z9 lemmas assembled per the
contract sketch; ℕ∞ case analysis via `ENat.ne_top_iff_exists`,
Data/ENat/Basic.lean:371).
-/
