...
3, the unassembled `log log (C·rⁿ) / log r → 0` asymptotic — the pin carries
no assembled lemma for it, re-confirmed by Annex A.5 and by the acceptance
panel) is assembled here from pinned pieces only:

Wording note: the sentence above deliberately avoids opening a line with the
word `lemma`. `scripts/gen_researchos_registry.py` scans built source for
declarations line-by-line without tracking `/- … -/` blocks, so a prose line
beginning with `lemma`/`theorem`/`def` inside a doc comment is read as a real
declaration — this exact sentence produced a phantom `Complex.exists` and
failed inverse coverage on the first promotion attempt. The scan is fail-safe
in the direction that matters (it can invent a declaration, never hide one),
so this is a wording constraint on comments, not a hole in the gate.
