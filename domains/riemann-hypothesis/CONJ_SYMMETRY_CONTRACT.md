...
Status: **DRAFT v1 (2026-08-06) — non-built review artifact. Not Lean-checked. No file in `Ecdlp/`, `ResearchOS/`, or any built target may be created from this document before independent review and the preconditions carried from `TARGET_BRIDGE_CONTRACT.md` / `XI_PACKAGE_CONTRACT.md` are met. The `S0-TRUST` prerequisite was satisfied by merged PR #298 (`d6e146fa`) on 2026-08-05. Adversarially reviewed once (verdict `SOUND_WITH_FIXES`: zero S0/S1/S2 — two LOW skeleton nits and two INFO locator cosmetics, all applied in this v2; see Annex B).**
...
- **Expected information gain.** Converts the map's `NOT-FOUND` row "conjugation symmetry for zeta/completion" into theorems; supplies the `ρ ↦ conj ρ` leg of the three symmetries required by `SOURCE_CONTRACTS.md` before `SC-LI-04`'s one-sided Li criterion; partially advances `S1-CONJ` (see boundary above). No information about the truth of RH is produced.
...
theorem riemannZeta_conj (s : ℂ) :
  ...
  · -- totalized point: conj 1 = 1, and ζ(1) is the coercion of a real number
    ...
    simpa only [riemannZeta_one, ofReal_log (by positivity : (0:ℝ) ≤ 4 * π), push_cast]  -- review fix F2: mirror ZetaAsymp.lean:434's one-step simp set
    push_cast
    rw [← Complex.conj_ofReal]                                          -- OBLIG S1C-3
    norm_num   -- exact cast walk to be fixed in build; mirrors ZetaAsymp.lean:434
  · -- identity theorem on {1}ᶜ; template: DirichletContinuation.lean:124-144
    ...
    have hg : AnalyticOnNhd ℂ (conj ∘ riemannZeta ∘ conj) ({1}ᶜ : Set ℂ) := by
      refine DifferentiableOn.analyticOnNhd (fun z hz => ?_) isOpen_compl_singleton
      have hz1 : (starRingEnd ℂ) z ≠ 1 := fun h => hz (by
        simpa [Complex.conj_conj] using congrArg (starRingEnd ℂ) h)
    ...
    have hfg : (conj ∘ riemannZeta ∘ conj) =ᶠ[𝓝 2] riemannZeta := by
      refine eventually_of_mem
        ((Complex.continuous_re.isOpen_preimage _ isOpen_Ioi).mem_nhds
          (by norm_num : (1:ℝ) < (2:ℂ).re)) (fun z (hz : 1 < z.re) => ?_)   -- OBLIG S1C-4
      ...
      rw [zeta_eq_tsum_one_div_nat_cpow hz', zeta_eq_tsum_one_div_nat_cpow hz,
        starRingEnd_apply, tsum_star]                                   -- OBLIG S1C-2
      congr 1 with n                                                     -- termwise
      -- star (1 / n ^ conj z) = 1 / n ^ z                               -- OBLIG S1C-1
      rw [← starRingEnd_apply, map_div₀, map_one]
      congr 1
      have harg : ((n : ℂ)).arg ≠ π := by
        rw [Complex.natCast_arg]; exact (Real.pi_ne_zero).symm
      calc (starRingEnd ℂ) ((n : ℂ) ^ (starRingEnd ℂ) z)
          = (starRingEnd ℂ) ((starRingEnd ℂ) ((starRingEnd ℂ) (n : ℂ) ^ z)) := by
            rw [← Complex.cpow_conj _ _ harg]
        _ = (n : ℂ) ^ z := by rw [Complex.conj_conj, Complex.conj_natCast]
...
theorem completedRiemannZeta₀_conj (s : ℂ) :
  ...
  have hfg : (conj ∘ completedRiemannZeta₀ ∘ conj) =ᶠ[𝓝 2] completedRiemannZeta₀ := by
    ...
    have key : ∀ w : ℂ, 1 < w.re →
      ...
      linear_combination h - hΛ  -- review fix F1 (sign)                 -- rearrangement; exact term fixed in build
    have hz' : 1 < ((starRingEnd ℂ) z).re := by rwa [Complex.conj_re]
    simp only [Function.comp_apply]
