# RH multiplicity / divisor theorem contract (S1-MULTIPLICITY): draft v1

Status: **DRAFT v1 (2026-08-06) — non-built review artifact. NOT Lean-checked.
No declaration below has been elaborated; no `lake build` has been run against
any of it. No file in any built target may be created from this document
before independent review, and nothing here counts as proved. Under the one
invariant, the Lean kernel via CI is the sole judge of every statement in this
contract. An internal adversarial review was run on the source skeleton
(verdict `SOUND_WITH_FIXES`, ten findings A1–A10, all applied in place; see
Annex A). That review accepts a statement surface only: it is not a kernel
verdict, it does not promote a module, and it does not close
`S1-MULTIPLICITY` or `S1-CONJ`. Statements M5–M8, M15, M17b and M17c are
additionally blocked on PR #306 (the conjugation package, in CI, not yet
merged on `main`).**

Working name: `Mult.lean` (module
`ResearchOS.AnalyticNumberTheory.RiemannHypothesis.Mult`).
Statement surface: **M1 – M17**.
...
Package prerequisites **not yet on `main`** — PR #306 (`Conj.lean`), in CI,
not merged. See §3:

| Symbol | Location | Status |
|---|---|---|
| `riemannZeta_comp_conj` | repo:`…/Conj.lean:163` | PR #306, in CI |
| `riemannXi_comp_conj` | repo:`…/Conj.lean:292` | PR #306, in CI |
| `analyticOrderAt_conj_conj` | repo:`…/Conj.lean:357` | PR #306, in CI |
| `analyticOrderAt_riemannZeta_conj` | repo:`…/Conj.lean:440` | PR #306, in CI |
| `analyticOrderAt_riemannXi_conj` | repo:`…/Conj.lean:452` | PR #306, in CI |

**Which statements need PR #306 and which do not.** `M1, M2, M3, M4, M9, M10,
M11, M12, M13, M14, M16, M16', M16''` and `M17a` are **independent of PR
#306** and can be landed against current `main` plus the pin. `M5, M6, M7, M8,
M15, M17b, M17c` are **package prerequisites on PR #306** and are unbuildable
until `Conj.lean` is on `main`. This is a sequencing constraint, not a
soundness defect.
...
its own close either barrier. `S1-CONJ` closes only if **both** this package
and PR #306 land, because M15/M17c themselves consume repo:`Conj.lean:440/452`.
...
- **Mechanism.** Pointwise `analyticOrderAt` is the primary carrier. The
  ...
  conjugation leg is imported wholesale from PR #306. The divisor half runs
  ...
  (NormalForm.lean:578), whose only hypothesis is pointwise finite order —
  supplied by M12 for ξ and by X11 + M12 for ζ on the strip.
...
- **Claim boundary.** M1–M4, M9–M14, M16–M16'' and M17a are unconditional
  ...
  `main`. M5–M8, M15, M17b, M17c additionally assume PR #306. Nothing touches
...
import ResearchOS.AnalyticNumberTheory.RiemannHypothesis.Conj   -- PR #306: Z9 legs
...
-- Analysis/Analytic/Order.lean:687, :693
...
-- Analysis/Complex/CauchyIntegral.lean:678, :625
...
`[#306]` requires PR #306 to land; `[GEN]` generic, natural Mathlib upstream.
...
M3; `analyticOrderAt_riemannXi_conj` repo:`Conj.lean:452` **(PR #306 —
package prerequisite, NOT pinned Mathlib)**.

### Obligations (M5)

None beyond PR #306 landing.
...
M4; `analyticOrderAt_riemannZeta_conj` repo:`Conj.lean:440` **(PR #306)**;
...
M3, M5; `analyticOrderAt_riemannXi_conj` repo:`Conj.lean:452` **(PR #306)**.
...
M4, M6; `analyticOrderAt_riemannZeta_conj` repo:`Conj.lean:440` **(PR #306)**.
...
```lean
  ...
  intro htop
  have hzero : riemannXi = 0 := (analyticOrderAt_eq_top_iff_eq_zero z hA).mp htop
...
- `analyticOrderAt_eq_top_iff_eq_zero` — `Mathlib/Analysis/Analytic/Order.lean:687`,
  verified verbatim: `[PreconnectedSpace 𝕜] {f : 𝕜 → E} (z : 𝕜)
  (hf : ∀ z₀, AnalyticAt 𝕜 f z₀) : analyticOrderAt f z = ⊤ ↔ f = 0`.
...
- **S1M-FIN** (**HIGH — the package's main obligation**). Without M12, the
...
- **S1M-12a** (MEDIUM): `PreconnectedSpace ℂ` must resolve by instance search
  ...
  **Fallback (corrected — finding A3).** Both fallbacks previously recorded
  here were defective. The valid route needs a *finite-order witness* first:
  ...
  **Two routes that do NOT work and must not be re-recorded (A3).**
  ...
  finite-order fact at the witness, which has to be produced from
...
with the same `Set.univ` and open-strip instances as M14 (the strip is
conj-symmetric because `((starRingEnd ℂ) z).re = z.re`, `Complex.conj_re`).

### Proof skeleton

Identical to M14 with `analyticOrderAt_riemannXi_conj` (repo:`Conj.lean:452`)
resp. `analyticOrderAt_riemannXi_one_sub_conj` (M5) in place of M3.

### Pinned dependencies (M15)

M5, M10, M14 pattern; `analyticOrderAt_riemannXi_conj`
repo:`Conj.lean:452` **(PR #306)**; `Complex.conj_re` —
`Mathlib/Data/Complex/Basic.lean:467`.

### Obligations (M15)

Same class as S1M-14a/S1M-14b, plus PR #306 landing.
...
### Block E — the ζ divisor on the open strip (exit item (d), ζ half) `[#306]` for M17b/c
...
finiteness from ξ to ζ inside the strip. No nonvanishing witness for ζ on Ω has
...
theorem riemannZeta_divisor_strip_conj (s : ℂ) :
    MeromorphicOn.divisor riemannZeta {z : ℂ | 0 < z.re ∧ z.re < 1} ((starRingEnd ℂ) s)
      = MeromorphicOn.divisor riemannZeta {z : ℂ | 0 < z.re ∧ z.re < 1} s          -- [#306]
...
**(PR #306)**; `Complex.conj_re` — `Mathlib/Data/Complex/Basic.lean:467` (the
...
## 3. PR #306 dependency map

**Package prerequisites, not pinned Mathlib.** These statements are blocked
until the conjugation package lands on `main`:

| Statement | Blocked on | Symbol consumed |
|---|---|---|
| M5 | PR #306 | `analyticOrderAt_riemannXi_conj` (repo:`Conj.lean:452`) |
| M6 | PR #306 | `analyticOrderAt_riemannZeta_conj` (repo:`Conj.lean:440`) |
| M7 | PR #306 | M5 + `analyticOrderAt_riemannXi_conj` |
| M8 | PR #306 | M6 + `analyticOrderAt_riemannZeta_conj` |
| M15 | PR #306 | M5 + `analyticOrderAt_riemannXi_conj` |
| M17b, M17c | PR #306 | `analyticOrderAt_riemannZeta_conj` (repo:`Conj.lean:440`) + M6 |

**M1–M4, M9–M14, M16, M16', M16'' and M17a are independent of PR #306** and can
be landed first against `main` + the pin. Recommended landing order:
...
*wait for #306* → `M5, M6, M7, M8, M15, M17b, M17c` (conjugation composite).

If PR #306 does not land, this package closes the reflection leg and the
divisor-object leg for both ξ and ζ, but **not** the exit: the exit phrase names
all three maps, and `ρ ↦ conj ρ` and `ρ ↦ 1 - conj ρ` both need #306. The
barrier row would then have to be updated to a partial exit, not a full one.
...
| `analyticOrderAt_eq_top_iff_eq_zero` | Mathlib/Analysis/Analytic/Order.lean:687 | M12 |
...
| **PR #306 (NOT on `main`)** `analyticOrderAt_riemannZeta_conj`, `analyticOrderAt_riemannXi_conj` | repo:`…/Conj.lean:440`, `:452` | M5–M8, M15, M17b/c |
...
- **No inference from an unproved fact.** X11 (repo:`Xi.lean:248`) is a
  kernel-checked repo theorem on `main`; PR #306's legs are declared as
...
- **No zero enumeration.** No index type, no ordering of zeros, no
  ...
  `locallyFinsuppWithin` object with *locally* finite support; on `Set.univ`
  and on the strip its support is infinite and it is never a `Finsupp`.
...
- **No growth, and none is hidden in the divisor.** The local-finiteness field
  ...
  Jensen inequality, no order-one bound, and nothing ζ/ξ-specific. M9/M16
  contribute only analyticity. Verified by reading the definition body at the
  pin (finding A10).
- **M12 and M16' are the only finiteness claims, and neither needs a zero-free
  region.** M12 comes from `riemannXi 0 = 1/2 ≠ 0` (repo:`Xi.lean:72`) plus
...
PR after independent review and after PR #306 lands for the `[#306]` items.
...
- `S1-MULTIPLICITY` exit item *"zeta/xi divisor interface"*: M9–M13 for ξ
  (`MeromorphicOn.divisor riemannXi U` evaluates by analytic order, is
  effective, has finite order everywhere, support is exactly the ξ zero set)
...
- **`S1-CONJ` closes only if both packages land.** PR #306 supplies the
  conjugation theorem; this package supplies the divisor invariance under
  `ρ ↦ 1 - conj ρ` (M15, M17c) — but M15/M17c *themselves* consume
  repo:`Conj.lean:440/452`. Neither package alone discharges `S1-CONJ`. This
...
- **No growth, and none is hidden in the divisor.** The local-finiteness field
  ...
  no order-one bound, and nothing ζ/ξ-specific. M9/M16 contribute only
  analyticity. Verified by reading the definition body at the pin.
- **M12 and M16' are the only finiteness claims, and neither needs a zero-free
  region.** M12 comes from `riemannXi 0 = 1/2 ≠ 0` (repo:`Xi.lean:72`) plus
  connectedness; M16' comes from M12 through X11. No lower bound on `|ζ|`, no
  nonvanishing witness on Ω, and no unproved analytic fact is used.
...
4. **A statement needs growth or counting bounds.** If any M requires a
   Jensen-type inequality, `logCounting`, an `N(T)` estimate, or a
   finite-order/Hadamard bound — stop.
...
8. **The barrier row is used as a target.** Per
   `repo/ECDLP_DECISION_SUBSTRATE.json` discipline: an open row is not by
   itself authorization to work a route. This contract is a statement design,
   not a claim that `S1-MULTIPLICITY` is the selected target.
...
on the working branch (`claude/rimmen-hypothesis-b6gd62`, HEAD `b04c089`).
...
discharged at :566–567), :614, :624, :687 (`analyticOrderAt_eq_top_iff_eq_zero`
verbatim), :693. `Calculus/InverseFunctionTheorem/Analytic.lean:40`.
...
- **Front 2 (hidden enumeration / counting / growth).** None found. See A10.
  ...
  hypothesis is pointwise finite order.
...
1. **PR #306 has not merged.** M5–M8, M15, M17b, M17c are unbuildable until
   `Conj.lean` is on `main`. Not a soundness defect; a sequencing blocker
...
4. **`repo/ECDLP_DECISION_SUBSTRATE.json` has not selected this route.**
   Death condition 8 stands: this document is a statement design only.
