...
| N7@small `n` | `x∘[n] = φₙ/ψₙ²` for fixed `n = 2,3,4,5` | closed `ring` identities | ✅ **`n = 2, 3` proved** — `secp256k1_double_x_eq_Φ₂_div_Ψ₂Sq` (`MultiplicationFormula.lean`), `secp256k1_triple_x_eq_Φ₃_div_ΨSq₃` (`TripleMultiplicationFormula.lean`, side-condition-free: the 2-torsion branch holds via `Ψ₂Sq ⊥ Ψ₃`); `n = 4, 5` open — same tangent/chord + certificate pattern |
