...
t0=time.time()
Awin=list(range(-3,4)); Twin=list(range(-3,4)); Pwin=list(range(-3,4)); Qwin=list(range(-3,4))
...
def gb_of(win,dic,par):
    rls=window_rules(dic,win,par,{'VS':True,'VSl3':True,'VG3':True,'VSlOdd3':True})
    g=[r_ for _,r_,_ in rls]
    return sp.groebner(g,*allgens,order='grevlex')
GA=gb_of(Awin,Ad,0); GT=gb_of(Twin,Td,0); GP=gb_of(Pwin,Pd,0); GQ=gb_of(Qwin,Qd,0)
print(f"4 window GBs built ({time.time()-t0:.0f}s)",flush=True)
def NF(x):
    x=sp.expand(x)
    for G in (GA,GT,GP,GQ):
        if x==0: return sp.Integer(0)
        _,x=G.reduce(x)
    # iterate to fixpoint (cross-window leftovers)
    for _ in range(4):
        prev=x
        for G in (GA,GT,GP,GQ):
            if x==0: return sp.Integer(0)
...
Enf=NF(E)
print(f"NF(E) terms {0 if Enf==0 else len(sp.Poly(Enf,*allgens).terms())} ({time.time()-t0:.0f}s)",flush=True)

# generators: couplings (not pre-paired) + pairing
Prng=list(range(-2,3)); Qrng=list(range(-2,3))
prules=pairing_rules({i:Psym[i] for i in Prng},{j:Qsym[j] for j in Qrng},
                     {i:Asym[i] for i in [-3,-2,-1,0,1,2,3]},{j:Tsym[j] for j in [-3,-2,-1,0,1,2,3]},0,0,Prng,Qrng)
gens=[('pair:'+n_,r_) for n_,r_,_ in prules]
st=(1,1,0); smt=(1,-1,0); sform=(1,0,0); tform=(0,1,0)
for lbl,(P1,P2) in {'Rst(':(sform,tform),'Rp m(':(st,smt)}.items():
    for u in range(-2,3):
        for v in range(-2,3):
            if u==v: continue
...
def wt(sym):
    for i in Awin:
        if sym==Asym[i]: return (1,0,0,0,i,0,0,0,i*i-1-3*e(i))
    for j in Twin:
        if sym==Tsym[j]: return (0,1,0,0,0,j,0,0,j*j-1-3*e(j))
    for i in Pwin:
        if sym==Psym[i]: return (0,0,1,0,0,0,i,0,i*i-1-3*e(i))
    for j in Qwin:
        if sym==Qsym[j]: return (0,0,0,1,0,0,0,j,j*j-1-3*e(j))
    if sym==B: return (0,0,0,0,0,0,0,0,12)
    if sym==c: return (0,0,0,0,0,0,0,0,8)
    if sym==d: return (0,0,0,0,0,0,0,0,12)
...
def poly_wts(expr):
    ...
    return ws
WE=poly_wts(Enf); 
print("NF(E) weights:",WE,flush=True)
assert len(WE)==1; WT=next(iter(WE))

symsW={'A':(Awin,Asym),'T':(Twin,Tsym),'P':(Pwin,Psym),'Q':(Qwin,Qsym)}
cols=[]
for nm,g in gens:
    wg=poly_wts(g)
    if len(wg)!=1: continue
    wg=next(iter(wg))
    dA,dT,dP,dQ,sIA,sIT,sIP,sIQ,K=tuple(a-b_ for a,b_ in zip(WT,wg))
    if min(dA,dT,dP,dQ)<0: continue
    def combos(win,dcount,ssum):
        for mm in itertools.combinations_with_replacement(win,dcount):
            if sum(mm)==ssum: yield mm
    for am in combos(Awin,dA,sIA):
     for tm in combos(Twin,dT,sIT):
      for pm in combos(Pwin,dP,sIP):
       for qm in combos(Qwin,dQ,sIQ):
...
tvec={m_:co for m_,co in sp.Poly(Enf,*allgens).terms()} if Enf!=0 else {}
...
if not sol:
    print("NO SOLUTION with couplings {R(s,t,u,v),R(s+t,s-t,u,v)}+pairing",flush=True)
else:
    ...
    print(f"uses {len(agg)} generators; sample:",flush=True)
    for nm in sorted(agg)[:24]:
