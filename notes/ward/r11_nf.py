...
# FULL in-window family R(s+i, t+j, u, v)
seen=set()
for i in range(-1,2):
 for j in range(-1,2):
  for u in range(-2,3):
   for v in range(-2,3):
    if (u,v)==(v,u) and u==v: pass
    key=(i,j,u,v)
    try: pol,ok=Rdef((1,0,i),(0,1,j),(0,0,u),(0,0,v))
    except Exception: continue
    if ok and pol!=0:
        gens.append((f"R(s{i:+d},t{j:+d},{u},{v})", sp.expand(pol)))
# and the sum/difference-center family R(s+t, s-t, u, v)
for u in range(-2,3):
 for v in range(-2,3):
  try: pol,ok=Rdef((1,1,0),(1,-1,0),(0,0,u),(0,0,v))
  except Exception: continue
  if ok and pol!=0:
      gens.append((f"Rp({u},{v})", sp.expand(pol)))
...
rows=[]
for m_ in allm:
    row={k_:Fr(int(pv[m_])) for k_,pv in enumerate(mats) if m_ in pv}
...
def add_row(r,rhs):
    ...
    while r:
        col=min(r)
        if col in pivots:
            pr,prhs=piv[pivots[col]]; f=r[col]/pr[col]
    ...
    return 'INC' if rhs!=0 else None
inc=False
for r,rhs in rows:
    if add_row(r,rhs)=='INC': inc=True; break
if inc:
    print("NO SOLUTION (inconsistent) within cofactor cap",flush=True)
else:
    x=[Fr(0)]*len(cols)
    for col in sorted(pivots,reverse=True):
        r,rhs=piv[pivots[col]]
        x[col]=(rhs-sum(v2*x[k2] for k2,v2 in r.items() if k2!=col))/r[col]
    vals=[sp.Rational(v.numerator,v.denominator) for v in x]
    combo=sum(vals[k_]*cols[k_][1]*cols[k_][2] for k_ in range(len(cols)) if vals[k_]!=0)
    print(f"SOLUTION; NF(E-combo)==0: {NF(E-combo)==0} ({time.time()-t0:.0f}s)",flush=True)
    import collections
