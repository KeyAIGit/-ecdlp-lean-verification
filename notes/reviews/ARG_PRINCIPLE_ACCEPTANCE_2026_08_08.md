...
Every enumerated finding now has an outcome; none is silent. **18 applied, 2 withdrawn, 3 stopped because they would have required a signature change.**
...
- **Truth lens 1, final clause of the proposed fix: 'rename A1 to match the pinned family, e.g. AnalyticAt.eventuallyEq_nhds_of_eventuallyEq_codiscreteWithin'.** — on `A1 — theorem AnalyticAt.eventuallyEq_of_codiscreteWithin (statement block in §2, unchanged)`.
  A declaration name is part of its signature: it sits between the `theorem` keyword and the `:=`. Renaming A1 changes the public surface, and the stage-one acceptance of 2026-08-08 is valid only for the surface as it stands, so applying the rename under an editorial task would silently invalidate the acceptance it claims to implement. STOPPED on this sub-fix; A1's signature is byte-identical. The substance of the finding — that A1 is a corollary of Meromorphic/IsolatedZeros.lean:99 and not a new move — is applied in full (locators, prose, three-line skeleton, HIGH → LOW), so only the cosmetic r
- **Truth lens 4, first option of the proposed fix: 'name it ..._of_notMem_closedBall_abs / ..._of_abs_lt_dist'.** — on `W1 — theorem circleIntegral.integral_sub_inv_of_notMem_closedBall (statement block in §2, unchanged)`.
  Same reason: the declaration name is part of the signature. STOPPED on the rename; W1's signature is byte-identical. The finding offered a second, non-signature repair — require a docstring on the built declaration stating the ball is closedBall c |R| and cross-referencing §5.2 — and that one IS applied, as a mandatory-docstring paragraph under W1's statement. Recorded in §5.4.
- **Truth lens 3, second half of the proposed fix: 'Either adopt the pin's R ≠ 0 + |R| convention across A2–A4 for consistency with W1 and with JensenFormula.lean:307, or add one sentence justifying the d** — on `A2, A3 and A4 — the three Complex.* statement blocks in §2, all unchanged`.
  Adopting the pin's convention would restate three conclusions and six hypotheses (divisor sums over ball c |R|, hypotheses over closedBall c |R| and sphere c |R|, and hR : R ≠ 0 in place of hR : 0 < R). That is three public signature changes. Note it would also be substantively wrong here without further work, since 0 < R is what makes hf₀ non-vacuous and hence what makes A2–A4 true at all (see the withdrawn A4 parenthetical above). STOPPED on the convention change; the finding's stated alternative — one sentence justifying the deliberate split — is applied instead, as a paragraph under A2 exp
...
- **Truth lens 2, cost half — '~3 lines from circleAverage_abs_radius'; 'promoting the Cauchy+flip route to primary retires two MEDIUM obligations (S1AP-W1a, S1AP-W1b) entirely'; and the proposed fix's 'p**
  PARTIALLY WITHDRAWN — the existence half is confirmed and fully applied; only the cost claim and the route demotion are declined. Reason, from reading the pin rather than the report: the only bridge at the pin between circleAverage and circleIntegral is Real.circleAverage_eq_circleIntegral (CircleAverage.lean:96), and it carries a (z - c)⁻¹ • weight. Recovering a bare ∮ means running it at the weighted integrand fun z ↦ (z - c) • g z at both radii and cancelling the weight along the path — and that cancellation cannot use circleIntegral.integral_congr (:425), whose 0 ≤ R excludes precisely the negative-radius case the flip exists to handle. It needs :430 (R ≠ 0) or an unfold, plus a smul/mul step and an R = 0 split (:422). So the derivation is short but real, and promoting the route does NOT retire two MEDIUM obligations — it substitutes a new one, registered as S1AP-W1e (MEDIUM). Demoti
- **Truth lens 3, parenthetical — 'A4 is not broken by this family; A4's 0 < R is convenience, A2/A3's is soundness.'**
  WITHDRAWN as incorrect, and replaced with the opposite conclusion. The record reached it after trying f = id and f = (·)², both of which yield nonnegative counts and so are satisfiable by some n : ℕ. But A4's ∃ n : ℕ also forbids a NEGATIVE count, and at R < 0 the hypotheses are vacuous so f is unconstrained and may have a pole. Counter-witness verified at the pin: f = fun z : ℂ => z⁻¹, c = 0, R = -1 gives ∮ = -(2πI) (deriv_inv, Deriv/Inv.lean:66; circleIntegral.integral_const_mul :527; integral_sub_center_inv :532), and 2πI·(n:ℂ) = -2πI has no natural-number solution. A4 is FALSE at R = -1. Applying the finding as written would have understated the death condition, so the contract now states that 0 < R is soundness-load-bearing in all three of A2, A3 and A4 without exception.

### Applied

- **Truth lens 1 — A1 re-pricing: A1 is a short corollary of a pinned lemma the contract never cited; S1AP-BRIDGE was wrongly HIGH and wrongly described as the package's gate.** — CONFIRMED at the pin myself before applying. Read MeromorphicAt.eventuallyEq_nhdsNE_of_eventuallyEq_codiscreteWithin at Mathlib/Analysis/Meromorphic/IsolatedZeros.lean:99 (namespace MeromorphicAt :31-:130; section variables :24-:27 identical to A1's binder context; conclusion PUNCTURED 𝓝[≠] x vs A1's UNPUNCTURED 𝓝 x), AnalyticAt.meromorphicAt (Meromorphic/Basic.lean:40), NormedField.nhdsNE_neBot (Normed/Field/Basic.lean:242, @[instance], [NontriviallyNormedField α]), Filter.Eventually.frequently (Order/Filter/Basic.lean:756), AnalyticAt.frequently_eq_iff_eventually_eq (Analytic/IsolatedZeros.lean:141). Applied: new §0 block quoting :99 plus the :59/:88/:109/:118 family and the three delta in
- **Truth lens 2 — the |R| flip the contract calls absent at the pin (existence half).** — CONFIRMED. Read Real.circleAverage_abs_radius at MeasureTheory/Integral/CircleAverage.lean:135 (@[simp], namespace Real :42-:382), with :129, :117, :54 and the bridge Real.circleAverage_eq_circleIntegral at :96; separately grepped neg_radius|abs_radius over the whole of CircleIntegral.lean and confirmed no ∮-level flip is named there (only circleMap_neg_radius :162 and circleIntegrable_neg_radius :292). Applied: new CircleAverage §0 block; the v1.1 sentence 'does not exist at the pin' explicitly withdrawn as misleading and replaced with 'present as mathematics, absent as a named ∮ lemma' in W1's alternative-route paragraph, in §0 and in DEFERRED-AP4; the Cauchy+flip route written out end to 
- **Truth lens 3 — 0 < R is load-bearing for TRUTH, not convenience; the trap is package-wide, not W1-local.** — CONFIRMED and SHARPENED. Verified Metric.closedBall_eq_empty (Pseudo/Defs.lean:468), sphere_eq_empty_of_neg (:442), ball_eq_empty (:387), circleIntegral.integral_sub_center_inv (CircleIntegral.lean:532, hypothesis exactly R ≠ 0) and integral_const_mul (:527). Applied package-wide as directed: a full soundness note under A2 with the f = id, c = 0, R = -1 witness; a matching note under A3 showing the iff fails (false LHS, vacuously true RHS); §5.2 extended from a W1-local remark to a package-wide subsection with a table of what is empty at R = -1 while the contour is not; death condition 8 split into (a) W1's premise-form and (b) weakening hR in A2/A3/A4; the four empty-region lemmas and deriv
- **Truth lens 3 (A4 sub-claim) — the record judged A4's 0 < R to be mere convenience.** — That sub-claim is WRONG and is corrected in place rather than propagated. The record tried only f = id and f = (·)², which produce nonnegative counts; but ∃ n : ℕ also forbids a negative count, and at R < 0 the hypotheses are vacuous so f need not be analytic. Witness verified at the pin: f = fun z : ℂ => z⁻¹, c = 0, R = -1. On the contour ‖z‖ = 1 so z ≠ 0, and deriv f z / f z = (-(z^2)⁻¹)/(z⁻¹) = -(z-0)⁻¹ by deriv_inv (Deriv/Inv.lean:66), giving LHS -(2πI) via :527 and :532; 2πI·(n:ℂ) = -2πI has no solution with n : ℕ. So A4 is FALSE at R = -1 too. Applied as a note under A4, in §5.2, and in death condition 8(b), each explicitly flagged as a correction to the acceptance record.
- **Truth lens 4 — W1's name records notMem_closedBall but not that the ball is closedBall c |R| (non-rename half).** — Applied the finding's second option: a mandatory-docstring paragraph under W1's statement requiring the built declaration to state that the excluded region is Metric.closedBall c |R|, the closed disc of geometric radius |R|, and to cross-reference §5.2. The rename option is withheld — see signature_change_required.
- **Truth lens 5 — extract_zeros_poles' docstring says 'open set U' while its binders carry no IsOpen.** — CONFIRMED, with the span corrected: the docstring is at FactorizedRational.lean:285-:290 (the record said :284-:290), and :291 is the theorem. Added a DOCSTRING WARNING note to the §0 FactorizedRational block stating that the binders govern, that U := closedBall c R is legal, and that a stage-two implementer must not stall on the docstring.
- **Pin lens 1 — §0's logDeriv_mul elision hides an EXPLICIT positional argument.** — CONFIRMED: logDeriv_mul {f g} (x : 𝕜) (hf) (hg) (hdf) (hdg) spans LogDeriv.lean:54-:56 with its proof at :57 (the record said :54-57). Both :54-:56 and logDeriv_prod :73-:75 are now quoted verbatim in §0 with all binders shown, prefaced by a note that (x : 𝕜) is explicit and precedes hf so a call written logDeriv_mul hφ hg … is wrong by one argument.
- **Pin lens 2 — S1AP-LOGD names the lambda-shape hazard only for logDeriv_fun_zpow, not one level up.** — CONFIRMED at the pin: logDeriv_mul concludes on logDeriv (fun z => f z * g z) x (:56) and logDeriv_prod on logDeriv (∏ i ∈ s, f i ·) x (:75), both lambda spellings, while A2 step 7 computes with the Pi-level product logDeriv (φ * g) z. S1AP-LOGD extended to name all three shapes (:56/:75/:87) and to record Pi.mul_apply / show as the discharge alongside the existing S1AP-SMUL. Resolved the record's open question about Pi.smul_apply: it and Pi.mul_apply are at Algebra/Notation/Pi/Defs.lean (namespace Pi :29-:165) — mul_apply declared at :70, and the non-primed smul_apply is the to_additive twin generated by the attribute line at :135 over Pi.pow_apply (:136), which is why it has no declaration
- **Pin lens 3 — consumed lemmas named in skeleton prose with no file:line, contrary to the house rule.** — Opened every one and added a new §0 block: Complex.abs_re_le_norm — Analysis/Complex/Norm.lean:38 (@[bound], namespace Complex :22-:389, with the RCLike homonym at RCLike/Basic.lean:690 flagged); HasDerivAt.comp — Deriv/Comp.lean:258 with x EXPLICIT via the trailing (x) of the variable block at :71 and its comment at :67-:68, which is what makes the skeleton's .comp z (…) correct; hasDerivAt_id — Deriv/Basic.lean:681 under variable (s x L) at :673; HasDerivAt.sub_const — Deriv/Add.lean:403, an alias of hasDerivAt_sub_const_iff (:400, (c : F)), which yields the (· - w) lambda shape; HasDerivAt.div_const — Deriv/Mul.lean:558 (mul_const :305 for the fallback); Int.cast_sum — Algebra/BigOperator
- **Pin lens 4 — the Divisor.lean namespace note is imprecise in a way that inverts the risk.** — CONFIRMED by reading Divisor.lean (namespace MeromorphicOn :28-:468): :68/:91/:104 take MeromorphicOn hypotheses and resolve by dot notation; :71 (AnalyticOnNhd.divisor_apply) and :177 (AnalyticOnNhd.divisor_nonneg) take AnalyticOnNhd hypotheses but sit inside namespace MeromorphicOn with no _root_, so dot notation does NOT resolve and the fully-qualified MeromorphicOn.AnalyticOnNhd.… is mandatory; only :83 is _root_-escaped. Verified the built precedent: Mult.lean:390 and :412 write exactly those two fully-qualified names. §0 note rewritten as a three-bullet split matching §3 point 1, and A4's dependency line updated to name the fully-qualified form.
- **Pin lens 5 — W1's bare `apply circleIntegral.integral_eq_zero_of_hasDerivWithinAt'` leaves the implicit primitive as a metavariable.** — Confirmed the shape at CircleIntegral.lean:538 (f is implicit and does not occur in the conclusion). Skeleton line replaced with refine circleIntegral.integral_eq_zero_of_hasDerivWithinAt' (f := fun u : ℂ => Complex.log ((u - w) / (c - w))) ?_, with a comment explaining that this is confirmed failure class (B) and that pinning the primitive before intro z hz is free. Also recorded as a one-line note under S1AP-W1a and in the register row.
- **Pin lens 6 — there is no top-level drafts/ directory; the drafts lane is domains/riemann-hypothesis/drafts/.** — Verified: no top-level drafts/, eleven files under domains/riemann-hypothesis/drafts/, and no ArgPrinciple.lean anywhere in the repo. Full path spelled at first use in the two-stage-gate paragraph with an explanation that the bare drafts/… shorthand is inherited from MULTIPLICITY_CONTRACT.md:17 and is a convention, not a path; also fixed at the Working-name line, in the name-collision scan (now naming all eleven files), in §What current CI does, and in the stage-two reference to drafts/README.md:32-:35. Annex A's two historical mentions left as the dated audit record they are.
- **Boundary lens 1 — the S1-GLOBAL-ZEROS disclaimer mischaracterises the row it disclaims.** — CONFIRMED by reading MATHLIB_CAPABILITY_MAP.md:387: the row blocks on 'no global enumeration, symmetric truncation, convergence, or counting API' and its exit-evidence column leads with 'finite divisor sums' — and A2-A4 are finite divisor sums, so the v1.1 'concerns zero enumeration' paraphrase is wrong. Replaced with the row's real text and the real reason: the sums here are of an ARBITRARY f over one disc, the row is scoped to this repository's ζ/ξ layer and additionally requires symmetric truncation, weighted summability, star convergence of Σ 1/ρ and the source-matched |ρ| ≤ T / |Im ρ| < T limits. Row untouched, stays OPEN, effect inventory only.
- **Boundary lens 2 — the contract negates only 'closes', never 'advances' / 'partially closes' / 'inventory only'.** — Adopted the repo's settled formula verbatim, verified at domains/riemann-hypothesis/drafts/README.md:33 (HarnackDisc) and :34 (PolyLiouville), with the equivalent statement in the built header at ResearchOS/Analysis/HarnackDisc.lean:23-:30 (the record cited :26-:31; the built header paraphrases rather than repeating verbatim, and I said so). Applied in two places: the §Scope paragraph and the first §Claim boundary bullet.
- **Boundary lens 3 — no repo-side destination and no ledger domain named for a stage-two promotion.** — CONFIRMED all three supporting facts, with the locators tightened: PREFIX_DOMAINS is a closed whitelist at scripts/gen_researchos_registry.py:46-:54 (nt-/RH-/MB-/HK-/PL-/TC-/GO- only), DOMAIN_SUBTREES at :60-:63, and the governing comment 'domain-neutral lemmas may not be filed inside a conjecture program's subtree' at :56-:59 — the record cited ':47-63' as one span. Also verified the drafts-lane promotion invariant at drafts/README.md:40-:45 and the four existing analysis-generic packages at :32-:35. Added a full subsection to §Stage two naming ResearchOS/Analysis/ArgPrinciple.lean as the destination, explaining why the RH subtree is forbidden, and stating that a new prefix (e.g. AP-) mappi
- **Boundary lens 4 — DEFERRED-AP5's unnamed 'the analytic case is the one with consumers in sight'.** — Clause deleted, and both of the finding's options taken: the sentence now ends '…gains a pole-factor case split. A future delta, not a redesign.', followed by a bracketed note recording why the clause was removed (in a route-neutral document an unnamed consumer invites the reader to supply ξ, which is the very analytic-vs-meromorphic split the sentence turned on) and naming the consumers that are actually in view, neutrally: DEFERRED-AP2, DEFERRED-AP3, and a possible upstream Mathlib submission.
- **Boundary lens 5 — no shape-neutrality statement; the route-neutrality argument is purely lexical.** — Added a Shape neutrality bullet to §Claim boundary. Locators corrected against the record: the RH-011 precedent is at tasks/RIEMANN_HYPOTHESIS.md:844 ('the no-cutoff-shape neutrality property re-verified mechanically') and :831-:832 ('an arbitrary compact K, with no cutoff shape anywhere, because choosing a cutoff shape is a route selection and all routes remain PARKED') — the record cited :790-:792, which is stale. The bullet states the two independent reasons the disc is not a cutoff choice here (no named function's zero set is truncated, and the disc is forced by the pin where circleIntegral is the only contour API), records the asymmetry rather than hiding it, points to ZERO_SET_SLICE_CO
- **Boundary lens 6 — 'Plus W1, a few-line library-gap closure' overclaims inside §Expected information gain.** — Replaced with 'a self-contained library-gap closure independent of the rest (few-line via Cauchy for 0 ≤ R; the uniform-sign statement adopted here costs more than "a few lines" on either recorded route — the log-primitive route carries S1AP-W1a/W1b and the Cauchy+flip route carries S1AP-W1e, see §2 W1 and §5.2)'. Adapted from the record's proposed wording because v1.2 no longer treats the primitive route as the only uniform option.
