...
- Kudo, Yokota, Takahashi, and Yasuda remains metadata-only. The row below
  deliberately records unknowns instead of reconstructing the paper from its
  title or abstract.
...
The auxiliary-curve construction remains parked because no exact retained
candidate closes its curve search, map, generalized-root, recovery, and
full-cost obligations. Kudo remains an explicit acquisition blocker. The
historical P3/P4 measurements cannot substitute for either faithful
construction.
