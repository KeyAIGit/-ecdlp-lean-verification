...
2. **Ward apparition rigidity** (`NormEDSConsecutiveZeros.lean`): over an integral
...
This module also discharges the registered N5 scalar-obligation target
`normeds_no_consecutive_zero` (`targets/normeds_no_consecutive_zero.json`): the same
nondegeneracy discharge, applied to the abstract rigidity theorem directly, yields
`secp256k1_normEDS_no_consecutive_zero` (last theorem below) — the scalar half stated in
its own right. Nothing downstream needs the scalar form (the node above consumes the
abstract theorem directly), but it is the exact statement the target registered, so it is
...
/-- **The N5 scalar obligation for secp256k1** (registered target
`normeds_no_consecutive_zero`): the curve-specialized scalar EDS
`w := normEDS β (Ψ₃ x₀) (preΨ₄ x₀)` over `𝔽̄_p`, with `β² = Ψ₂Sq(x₀)`, has no two
consecutive zeros. A direct instance of Ward apparition rigidity
(`Ecdlp.NormEDS.normEDS_not_consecutive_zeros`) whose two nondegeneracy side conditions are
the L5/L6 certificates at the closure — the same discharge `secp256k1_isCoprime_Φ_ΨSq`
performs inline. The hypothesis `1 ≤ n` is kept only to match the registered stem
signature; the statement holds for every `n : ℤ`. -/
theorem secp256k1_normEDS_no_consecutive_zero
    (x₀ β : AlgebraicClosure (ZMod Secp256k1.p))
    (hβ : β ^ 2 = secp256k1Bar.Ψ₂Sq.eval x₀) {n : ℤ} (_hn : 1 ≤ n) :
  ...
  refine Ecdlp.NormEDS.normEDS_not_consecutive_zeros β
    (secp256k1Bar.Ψ₃.eval x₀) (secp256k1Bar.preΨ₄.eval x₀) ?_ ?_ n
  · rintro ⟨hb, hc⟩
    have hΨ2 : secp256k1Bar.Ψ₂Sq.eval x₀ = 0 := by rw [← hβ, hb]; norm_num
    exact no_common_root secp256k1Bar_isCoprime_Ψ₂Sq_Ψ₃ hΨ2 hc
  · rintro ⟨hc, hd⟩
