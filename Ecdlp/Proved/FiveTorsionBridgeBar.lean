...
5x¹² + 2660x⁹ − 11760x⁶ − 548800x³ − 614656 = 0`.

This is the port of `Ecdlp/Proved/FiveTorsionBridge.lean` from `𝔽_p` to `𝔽̄_p`, exactly
as `ThreeTorsionBridgeBar.lean` ported `ThreeTorsionBridge.lean` — and it is **the
missing link for the count `#E[5](𝔽̄_p) = 25`** (node N11 at `n = 5` of
`notes/DIVISION_POLY_TORSION_MAP.md`). Over `𝔽_p` itself the bridge is vacuous for
counting: the secp256k1 point group has prime order `n ≠ 5`, so `E[5](𝔽_p) = {O}` and
the twelve roots of `preΨ₅` do not lie on `𝔽_p`-rational points. The genuine content
lives over the closure, where `preΨ₅` has exactly `12` distinct roots
(`secp256k1_preΨ₅_roots_card_bar` / `secp256k1_preΨ₅_roots_nodup_bar` in
`DivisionPolynomialSeparable.lean`); each root `x₀` carries two points `(x₀, ±y₀)`, so
this bridge turns the root count `12` into the point count `#E[5](𝔽̄_p) = 2·12 + 1 = 25
= 5²` — assembled in `FiveTorsionStructure.lean`.
...
`5x¹² + 2660x⁹ − 11760x⁶ − 548800x³ − 614656 = 0`). This is where the `ψ₅ ↔ E[5]`
bridge has its real content: over `𝔽̄_p` the twelve roots of `preΨ₅` are realized by
actual curve points, giving `#E[5](𝔽̄_p) = 25` once paired with `±y`. -/
...
(`12`, all distinct) by `secp256k1_preΨ₅_roots_card_bar`/`…_nodup_bar`. Together they say:
the 5-torsion `x`-locus of `secp256k1Bar` is exactly that 12-element root set. -/
