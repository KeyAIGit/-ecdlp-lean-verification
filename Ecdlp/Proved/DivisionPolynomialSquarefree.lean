...
and **`ψₙ` squarefree** is separability of `[n]` in polynomial clothing — the last missing
input of `#E[n] = n²` along route B of `notes/SEPARABILITY_ROUTES.md` (node **B4**). A
squarefree `ψₙ` of degree `(n² − 1)/2` has exactly `(n² − 1)/2` distinct roots over `𝔽̄_p`,
i.e. the `n`-torsion `x`-locus is as large as it can be; combined with the degree facts
(`DivisionPolynomialDegree.lean`) this yields the exact root counts feeding `#E[n] = n²`.
...
`n = 3`). Combined with `deg Ψ₃ = 4` (`DivisionPolynomialDegree.lean`) this says the
3-torsion `x`-locus has exactly `4 = (3² − 1)/2` distinct points over `𝔽̄_p` — the root-count
input to `#E[3] = 3²`. Explicit Bézout certificate over `𝔽_p`; the analogue of the
separability of `[3]`. -/
...
is coprime to its own derivative** (B4 at `n = 5`). With `deg = 12 = (5² − 1)/2` this gives
exactly 12 distinct 5-torsion `x`-coordinates over `𝔽̄_p` — the root-count input to
`#E[5] = 5²`. Explicit Bézout certificate over `𝔽_p`; the analogue of the separability
of `[5]`. -/
...
is coprime to its own derivative** (B4 at `n = 7`). With `deg = 24 = (7² − 1)/2` this gives
exactly 24 distinct 7-torsion `x`-coordinates over `𝔽̄_p` — the root-count input to
`#E[7] = 7²`. Explicit Bézout certificate over `𝔽_p`; the analogue of the separability
of `[7]`. -/
theorem secp256k1_isCoprime_preΨ₇_derivative :
