...
/-!
# Separability of the division polynomials, and their distinct-root counts over `𝔽̄_p`
...
* `ψₙ` has **exactly `(n²−1)/2` distinct roots in `𝔽̄_p`** for `n = 3, 5, 7`
  (`…_roots_card_bar` + `…_roots_nodup_bar`). This is a statement purely about the
  polynomial `ψₙ` and the multiset of its roots in `𝔽̄_p`; it is the counting input of
  node **N11** at small `n` (`notes/DIVISION_POLY_TORSION_MAP.md`).

*Deferred / downstream (not proved here).* Identifying these roots with the `n`-torsion
`x`-locus, and — together with the `±y`-pairing (N12) and the point-level bridge
transported to the closure — pinning `#E[n](𝔽̄_p) = n²`, is the next rung, handled in
later files; none of it is established in this one.
...
are distinct, `secp256k1_Ψ₃_roots_nodup_bar`), i.e. `Ψ₃` has exactly 4 distinct roots in
`𝔽̄_p`. (Downstream, not here: identifying these with the 3-torsion `x`-locus.) -/
...
(Downstream, not here: identifying these with the 5-torsion `x`-locus.) -/
...
`secp256k1_preΨ₇_roots_nodup_bar`), i.e. `preΨ₇` has exactly 24 distinct roots in `𝔽̄_p`.
(Downstream, not here: identifying these with the 7-torsion `x`-locus.) -/
theorem secp256k1_preΨ₇_roots_card_bar :
