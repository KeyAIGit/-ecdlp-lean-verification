...
/-- **Point-level 2-torsion criterion over `𝔽̄_p`: `2 • P = 0 ⟺ y = 0`.** The port of
`secp256k1_two_nsmul_eq_zero_iff` (`TwoTorsionPoint.lean`) to `secp256k1Bar`:
`2 • P = P + P = 0 ⟺ P = -P ⟺ y = negY x y = -y ⟺ 2y = 0 ⟺ y = 0` (char `≠ 2`). No
multiplication formula — this is the whole even bridge. -/
theorem secp256k1Bar_two_nsmul_eq_zero_iff
  ...
  constructor
  · rintro ⟨-, hy⟩
    have h2y : (2 : AlgebraicClosure (ZMod Secp256k1.p)) * y = 0 := by linear_combination hy
    rcases mul_eq_zero.mp h2y with h2 | hy0
    · exact absurd h2 two_ne_zero_bar
    · exact hy0
  · intro hy
    subst hy
    exact ⟨rfl, by ring⟩
