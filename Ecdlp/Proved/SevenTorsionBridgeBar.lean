...
univariate `7x²⁴ + 27608x²¹ − ⋯ + 377801998336` vanishes at `x`. That per-point
equivalence is exactly the headline theorem `secp256k1Bar_seven_nsmul_eq_zero_iff`, and is
all this file proves.

This is the port of `Ecdlp/Proved/SevenTorsionBridge.lean` from `𝔽_p` to `𝔽̄_p` — the `n = 7`
analogue of `ThreeTorsionBridgeBar.lean`.

*Motivation / downstream (not proved in this file).* Combining this per-point bridge with
the closure root-count — the mapped `preΨ' 7` has exactly `24` distinct roots in `𝔽̄_p`
(`secp256k1_preΨ₇_roots_card_bar` / `…_nodup_bar`, `DivisionPolynomialSeparable.lean`) —
and the `±y`-pairing (each nonzero root carrying two points `(x₀, ±y₀)`) is what would turn
the root count `24` into a point count `2·24 + 1 = 49 = 7²` (node N11 at `n = 7` over the
closure). That assembly — including surjectivity onto the root set and the `y₀ ≠ 0` input
— is done in `SevenTorsionStructure.lean`, not here.
...
For a nonzero affine point `P = (x, y)` of `secp256k1Bar`, the group relation `7 • P = 0`
holds iff the 7-division polynomial vanishes at `P`. This per-point equivalence is the
full content of the theorem. (Downstream, not proved here: that the `24` roots of the
mapped `preΨ' 7` are each realized by an actual curve point and that, paired with `±y`,
this yields `#E[7](𝔽̄_p) = 49`, as assembled in `SevenTorsionStructure.lean`.) -/
...
(`24`, all distinct) by `secp256k1_preΨ₇_roots_card_bar`/`…_nodup_bar`. This equates the
`x`-coordinate of any 7-torsion point with a root of that polynomial. (Downstream, not
proved here: the reverse identification — that the 7-torsion `x`-locus is *exactly* that
24-element root set — additionally needs surjectivity, i.e. that every root is realized by
