...
- **L1 Verified core** — kernel-checked theorems (`VERIFIED.md`, `Ecdlp/Proved/*`). The
  ground truth. *Status: 116 rows / ~106 distinct, 0 sorry, no custom axioms.*
...
- **L4 Formalized objects** — GLV endomorphism (now a verified primitive cube root of
  unity), torsion, division polynomials; the pairing (missing) — the machinery reasoning
  runs on.
...
generator (corpus → target stems), the prover loop (Tier-0 + model tiers), the CI trust
gates (no-sorry, axiom audit, count/import guards). Optional: a warm server node (results
